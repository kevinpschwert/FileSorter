...
namespace FileSorter.Helpers
{
    ...
    {
        public static bool ValidateConsolidatedActualFiles(string destinationPath, ArrayOfExportFileMetadata files, List<string> unzipped)
        {
            foreach (var file in files.ClientFiles)
            {
                string clientFolder = $"{file.EntityName} - {file.EntityID}";
                // If the folder in not in ConsolidatedData, create the folder
                ...
                if (file.Class == "Permanent")
                {
                    ...
                    string yearFilePath = FindActualFolder(unzipped, clientFolder, folderName, file.VirtualFolderPath, file.FileName);
        ...
        public static void ConsolidateFilesNew(string destinationPath, List<Files> files, List<string> unzipped)
        {
            try
            {
                foreach (var file in files)
                {
                    string folderName = $"{file.ClientName} - {file.EntityId}";
                    // If the folder in not in ConsolidatedData, create the folder
                    string clientName = Path.Combine(destinationPath, folderName);
                    if (!Directory.Exists(clientName))
                    {
                        Directory.CreateDirectory(clientName);
                    }
                    // If the year folder is not in the client folder, create the year folder
                    if (!Directory.Exists(Path.Combine(clientName, file.Year.ToString())))
                    {
                        Directory.CreateDirectory(Path.Combine(clientName, file.Year.ToString()));
                    }

                    string yearFilePath = FindFolderNew(unzipped, folderName, file.Year.ToString(), file.FileName);
                    string consolidatedYearFilePath = Path.Combine(destinationPath, folderName, file.Year.ToString());
                    if (!System.IO.File.Exists(Path.Combine(consolidatedYearFilePath, file.FileName)))
                    {
        ...
        public static void ConsolidateActualFiles(string destinationPath, ArrayOfExportFileMetadata files, List<string> unzipped)
        {
            try
            {
                foreach (var file in files.ClientFiles)
                {
                    if (file.FileName == "09-2022 Bitcoin - Awan.pdf")
                    {
                        bool isIt = true;
                    }
                    string clientFolder = $"{file.EntityName} - {file.EntityID}";
        ...
        private static string FindActualFolder(List<string> unzip, string companyName, string year, string virtualFolderPath, string fileName)
        {
            foreach (var file in unzip)
            {
                //string yearFilePath = $"C:\\Users\\kevin\\OneDrive\\Desktop\\NewClientIdData\\{file}\\{companyName}\\{year}";
